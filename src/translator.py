...
# Carpeta donde guardaremos los modelos extraídos
if getattr(sys, 'frozen', False):
...
class Translator:
    def __init__(self):
        ...
        # Diccionarios de idiomas y estado
        self._available_languages = {}
        self._argos_index = [] # JSON de internet
        self._installed_pairs = set() # set de tuplas ("en", "es")
        
        # Caché de modelos cargados en memoria RAM para no recargarlos en cada frase
        # key: "en_es", value: (ctranslate2.Translator, spm.SentencePieceProcessor)
        self._loaded_models = {}
        
        self._load_catalogs()

    def _load_catalogs(self):
        # Carga el catálogo local (idiomas base)
    ...
    def _initialize(self):
        try:
            ...
            # Descargar catálogo actualizado en segundo plano
    ...
    def _fetch_argos_index(self):
        try:
            ...
            # Actualizar available_languages con lo que encontramos en internet
    ...
    def refresh_languages(self):
        # Revisa qué carpetas hay en models/argos_ct2
        ...
        for folder in os.listdir(MODELS_DIR):
            ...
            if os.path.isdir(folder_path):
                # La convención que usaremos será fromCode_toCode, ej: "en_es"
    ...
    def is_model_installed(self, from_code, to_code):
        ...
        # Chequeo de puente (pivot) por inglés
    ...
    def _get_model(self, from_code, to_code):
        # Devuelve el (translator, sp_processor) para el par
        ...
        # En el ZIP, Argos guarda los archivos en subcarpetas con versiones.
        ct2_dir = None
        sp_path = None
    ...
    def _translate_direct(self, text, from_code, to_code):
        ...
        if not translator or not sp_processor:
            logger.error(f"DEBUG: No se pudo obtener el modelo para {from_code}->{to_code}")
            return text
        
        try:
            # Dividimos por saltos de línea para traducir párrafo a párrafo (Argos sentencizer no está, esto es básico y rápido)
            lines = text.split('\n')
            translated_lines = []
    ...
    def download_model(self, from_code, to_code, progress_callback=None):
        try:
            ...
            # Determinar qué paquetes bajar (directo o puente)
            needed = []
            ...
            for i, pkg in enumerate(needed):
                ...
                if (fc, tc) in self._installed_pairs: continue # Ya lo tenemos
    ...
    def delete_model(self, from_code, to_code):
        try:
            # Borrar directo o puente
            paths_to_delete = []
            dir_direct = os.path.join(MODELS_DIR, f"{from_code}_{to_code}")
            if os.path.exists(dir_direct): paths_to_delete.append(dir_direct)
            
            # Si usa puente, borramos ambos solo si están instalados (puede que se usen para otros, pero para simplificar...)
            # En la vida real, borrar el puente puede romper otros idiomas. Para mantenerlo seguro, borramos solo si existe el par explícito.
            # Mejor borramos solo lo que nos pidan específicamente, o vaciamos caché
